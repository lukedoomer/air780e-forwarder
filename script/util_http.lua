...
-- @param headers 请求头
-- @param body 请求体
function util_http.httpWithTimeout(timeout, method, url, headers, body)
    ...
    local res_code, res_headers, res_body = 0, {}, ""

    util_netled.blink(50, 50)

    sys.taskInit(
        function()
            log.debug("util_http.httpWithTimeout", "开始请求", "id:", id)
            res_code, res_headers, res_body = http.request(method, url, headers, body, opts).wait()
            log.debug("util_http.httpWithTimeout", "请求结束", "id:", id, "code:", res_code)
            sys.publish(id)
        end
    )
    local result = sys.waitUntil(id, timeout)
    if not result then
        log.warn("util_http.httpWithTimeout", "请求超时", "id:", id)
    end
