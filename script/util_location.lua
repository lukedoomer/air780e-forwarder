...
local function getMapLink(lat, lng)
    ...
    if lat ~= 0 and lng ~= 0 then
        map_link = "https://apis.map.qq.com/uri/v1/marker?coord_type=1&marker=title:+;coord:" .. lat .. "," .. lng
    end
    log.info("util_location.getMapLink", map_link)
    return map_link
end
...
function util_location.refresh(timeout, is_refresh_cell_info_disabled)
    ...
    local openluat = function(cell_info_formatted)
        ...
        local code, headers, body = util_http.httpWithTimeout(timeout, "POST", lbs_api, header, body)
    ...
    local cellocation = function(cell_info_formatted)
        ...
        local code, headers, body = util_http.httpWithTimeout(timeout, "GET", lbs_api)
        log.info("util_location.refresh", api_type .. ":", "code:", code, "body:", body)
